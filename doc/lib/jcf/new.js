...
// scope 내부에서 해당 id 값을 찾아 반환 음..
service.sel = function() {
...
var dialogController = ['$scope', 'dialogParam', 'model', 'model2', function($scope, dialogParam, model, model2){
  ...
  function ($http, $document, $compile, $rootScope, $controller, $templateCache, $q, $transition) {
    ...
    return {
      ...
      messageBox: function(title, message, buttons){
        return new Dialog({templateUrl: 'template/dialog/message2.html', controller: 'dialogController', resolve: {model: {
          title: title,
          message: message,
          buttons: buttons
        }, model2:{test:"test"}}
...
var jcfNew = angular.module('jcf.new', ["jcf.template", "ui.bootstrap.transition"]).directive(directive).factory(service);

jcfNew.controller("dialogController", dialogController)
.provider("$dialog2", dialog2);
